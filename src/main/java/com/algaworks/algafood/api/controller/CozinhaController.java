...
// 		Caso não tenha dados retorna 404 NotFound		
		if (cozinha != null) {
			return ResponseEntity.ok(cozinha);					
		}
		
//		return ResponseEntity.status(HttpStatus.NOT_FOUND).build(); - mesmo do de baixo
...
//	@ResponseStatus(HttpStatus.CREATED) cria status http 
/*	@GetMapping("/{cozinhaId}")
	public ResponseEntity<Cozinha> buscar(@PathVariable Long cozinhaId) {
		Cozinha cozinha = cozinhaRepository.buscar(cozinhaId);	
		
		// return ResponseEntity.status(HttpStatus.OK).body(cozinha);
		// return ResponseEntity.ok(cozinha); // mesmo que o de cima
		
		HttpHeaders headers = new HttpHeaders();
		headers.add(HttpHeaders.LOCATION,"http://api.ricardo.local:8080/cozinhas");
		
		return ResponseEntity
				.status(HttpStatus.FOUND) // retorna o 302
				.headers(headers)
				.build();		
	}	*/
